...
Perseus, Della, Tiger and Traverse all mount the tigress file system. If you have an account on two or more of these clusters it is recommended that you store your aliases and shell functions (see next section) in a file on tigress and source this from each `.bashrc` file on the clusters that you have access to.

For example, make the file `/tigress/<NetID>/my_aliases.bash` and put your aliases there. Then add this line of each of your `.bashrc` files:
